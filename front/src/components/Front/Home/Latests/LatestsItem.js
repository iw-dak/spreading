...
const LatestItem = ({ latest }) => <>
    <div className={`col-12 mb-2 mb-lg-0 col-lg-6 latest-item latest-item-${latest.id}`}>
        <img className="latest-item-image" src={`${latest.image}?t=${Date.now()}`} alt={latest.title} />
        <div className="latest-item-wrapper">
            <a href={process.env.REACT_APP_URL + '/article/' + latest.slug}>
                ...
                <div className="latest-post-title">{latest.title}</div>
            </a>
        </div>
    </div>
</>
