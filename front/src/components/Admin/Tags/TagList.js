...
class TagList extends Component {
  ...
  remove(e, tag) {
    myApi.endpoints.tags.delete({ id: tag.id }).then(({ data }) => {
      this.setState({
        alertDeleted: 'Le tag a bien été supprimé.'
      })
    }).catch(error => {
      console.log("error", error);
    });
              ...
              <div className="modal-body">
                <form onSubmit={(e) => this.add(e)}>
                  <div className="form-group">
                    ...
                    <br />
                    <input type="text" className="form-control" aria-describedby="tag" placeholder="Label du tag" name="slug" onChange={this.handleChange} />
                  </div>
